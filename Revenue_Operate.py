...
class RevenueAllocator:
    ...
    def _get_hours_file_path(self, date):
        """
        根据日期生成工时数据文件路径
        ...
        return f"hours_{month_str}.csv"

    def _load_hours_data(self, date):
        """
        加载指定月份的工时数据
        :param date: datetime.date对象，用于确定月份
        """
        self.hours_file = self._get_hours_file_path(date)
    ...
    def allocate_working_hours(self, results, max_hours_per_day, start_date, end_date, staff_dict):
        """
        ...
        """
        # 加载现有工时数据（使用开始日期确定月份）
        self._load_hours_data(start_date)
